module rates_nn
    ...
    function calculate_w0(parameters::sim_parameters,states::sim_state)
        function prob(i)
            k::Int64=abs(i)
            p=0
            for r::Int64 in k:floor(states.max_neigbours/2+k/2)#range(k,int(np.floor(nn/2+k/2)+1)):
                g::Int64=r-k
                p+=binomial(states.max_neigbours, r+g)*(2/parameters.number_of_states)^(r+g)*(1-2/parameters.number_of_states)^(states.max_neigbours-r-g)*binomial(r+g, r)*(1/2)^(r+g)
            end
            return p
        ...
        w0=0
        for i in -states.max_neigbours:states.max_neigbours
            w0+=prob(i)*(states.rates_function[i+states.max_neigbours+1,1]-states.rates_function[i+states.max_neigbours+1,2])
    ...
    function glauber_rates(parameters::sim_parameters,states::sim_state) #:: Function
        ...
        function rates_f(d::Int64,s::Int64) :: Float64
            return 2 / tau * exp(s * beta / 2 * driving_force)/(exp(-interaction_strength/max_neigbours *
             d)+1)
        end
